...
def main(args):
	...
	#
	# if locus level gene counts will be found here we check on the status of the bowtie index for 
	# the transcriptome
	#
	if not args.no_counts:
		if len(args.tref) == 0:
			# 
			# no transcriptome index provided so we have to build one
			#
			sys.stderr.write("> building bowtie index for transcriptome alignments...\n")
			sp.call("gffread -g {:s} -w transcriptome.fa {:s}".format(args.genome_ref, args.gtf), shell=True)
			fnull = open("/dev/null","w")
			p1 = sp.Popen("bowtie-build -o 1 transcriptome.fa transcriptome".split(), stderr=fnull, stdout=fjull)
			p1.wait()
			fnull.close()
			sp.call("rm transcriptome.fa", shell=True)
			args.tref = "transcriptome"
			
			

	#
	# now we need to parse the GTF in so we have all of its information ready to roll
	#
	...
	al_bed_out = open("splice_pos.bed", "w")
	al_idx = 0
	...
	for rloc in sorted(locus_rows.keys()):
		...
		if len(junctions_to_exons.keys()) > 0:
			...
			for key in junctions_to_exons.keys():
				...
				# write bed line for the splice position in this feature
				tstart = alocs[alid]['skip']['splice_pos']-overlap_window
				tend = alocs[alid]['skip']['splice_pos']+overlap_window
				if tstart < 0:
					tstart = 0
					
				al_bed_out.write("\t".join(map(str, [alid + ".skip", tstart, tend])) + "\n")
				...
				for eid in elist:
					...
					for jid in jlist:
						...
						# write the bed line for the splice position
						tstart = alocs[alid]['exons'][al_ei][tidx]['splice_pos']-overlap_window
						tend = alocs[alid]['exons'][al_ei][tidx]['splice_pos']+overlap_window
						if tstart < 0:
							tstart = 0
							
						al_bed_out.write("\t".join(map(str, [alid + "." + al_ei + "." + str(tidx),tstart, tend])) + "\n")
	...
	for rfile in args.read_files:
		...
		if not args.no_counts:
			sys.stderr.write("> doing transcriptome counts analysis...\n")
			p1 = sp.Popen("transcriptome-counts -p {:d} {:s} {:s} {:s}".format(args.num_threads, args.tref, args.gtf, rfile).split(), stderr=fnull)
			p1.wait()

		sys.stderr.write("> aligning {:s} to splice positions with bowtie...\n".format(rfile))
		...
		if m:
			sp.call("bash -c 'bowtie --offrate 1 -t -p {:d} -a -n 1 -l 25 -e 60 -S alt_locs <(gunzip -c {:s}) | samtools view -bS -F 0x04 -o temp0.bam -'".format(args.num_threads,rfile),shell=True)
		else:
			sp.call("bash -c 'bowtie --offrate 1 -t -p {:d} -a -n 1 -l 25 -e 60 -S alt_locs {:s} | samtools view -bS -F 0x04 -o temp.bam -'".format(args.num_threads,rfile),shell=True)


		#
		# intersect alignments with the splice_pos.bed file. parse out the hits that have 'overlap_window x 2' overlap
		#
		
		sys.stderr.write("> counting hits at splice positions...\n")
		
		# repository hash for counts at each alid
		aloc_counts = {}
		
		# do intersection with bedtools
		p1 = sp.Popen("bedtools intersect -bed -wo -abam temp0.bam -b splice_pos.bed".split(), stdout=sp.PIPE)
		...
		for szl in p1.stdout:
			...
			if len(szl) > 0:
				...
				if int(arl[9]) == overlap_window*2:
					
					if arl[0] not in aloc_counts:
						aloc_counts[arl[0]] = 0
		...
		#
		# now using the alocs strucure we can organize the counts information in alocs_counts into a coherent output
		#
			...
			for i in range(num_exons):
				...
				for j in range(num_juncs):
					...
					if al_key in aloc_counts:
						...
						p_counts.append(aloc_counts[al_key])
					else:
						#p_counts += [0,0]
						p_counts.append(0)
					#}
	...
	# clean up
	p1 = sp.Popen("rm temp0.bam temp.exons temp.juncs temp_locus.gtf".split())
	p1.wait()
...
parser.add_argument('-t', dest='tref', type=str, default="", help="Transcriptome bowtie index (default: build one from provided GTF)")
...
parser.add_argument("--no-counts", dest="no_counts", default=False, action='store_const', const=True, help="Skip transcriptome alignment to find gene counts (default: perform analysis)")
