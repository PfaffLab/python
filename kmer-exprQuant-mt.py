...
def main(args):
	...
	refin = args.ref
	if args.r_stranded:
		cmd = "reformat.sh in={} rcomp out={}_ref.fa overwrite".format(args.ref, stub)
		rres = runcmd(cmd, False)
		refin = "{}_ref.fa".format(stub)

	# -------------------------------------------------------------------------
	# run Seal. output goes to fifo. the loop reads lines and sends
	...
	sys.stderr.write("Starting {} worker threads\n".format(args.t))
	for i in range(args.t):
		worker = Thread(target=kmer_worker_main, args=(i, annot, hit_queue, rres_queue,))
	...
	for szl in fifoin:
		...
		if rres:
			...
			if len(lparsed) >= parse_lim:
				sys.stderr.write("parsed {} lines\n".format(nlines))
				# pass the parse queue to the hit queue for processing by workers
				sys.stderr.write("...handling hits...")
				for i in range(len(lparsed)):
					hit_queue.put(lparsed[i])
	...
	while not result_queue.empty():
		lres = result_queue.get()
		for hit in lres:
			annot[hit[i][0]] += hit[i][1]
			total_depth += hit[i][1]
...
#
# main worker function for dealing with a single matched kmer line.
# this will push a list into an output queue
def kmer_worker_main(wid, annot, qin, qout):
	...
	while True:
		...
		# loop through targets and adjust hits
		for i in range(len(targets)):
			targets[i][1] *= tgid_weight

		# put target list into output queue
		qout.put(targets)
...
parser.add_argument('-t', type=int, default=2, 
	help="Number of worker threads for handling matched kmers [2]")
