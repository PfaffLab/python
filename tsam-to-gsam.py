	...
	for szl in fin:
		...
		if len(szl) > 0 and szl[0] == "@":
			...
			if not paired and flag & 0x4:
				...
				# either read is aligned or its a paired read that isn't aligned...
				...
				if flag & 0x4:
					...
					# this is an aligned read either paired or single-end. now the 
					# alignment information can be modified from transcriptome to 
					# genome
...
def are_paired(r1, r2):
	...
	fns = ((flag1 & 0x40) and (flag2 & 0x80)) or ((flag1 & 0x80) and (flag2 & 0x40))
	possible_mates = r1[0] == r2[0] and fns
	possible_discordant = r1[6] != "=" or r2[6] != "="

	if r1[0] != r2[0]:
		# different query names
		return -1
	#}
	
	if not fns:
		# alignments aren't mates
		return -1
...
parser.add_argument("sam", type=str, help="Alignments in SAM format or - to read from STDIN")
