...
# this pipeline bundles alignments by read name and then looks up
# intersections and deals with those intersections.  
#==============================================================================
...
#				continue
			...
			if (rname != rname_last) and (len(aln_buffer) > 0):
				...
				#
				# check buffer. if reads are printed in a SAM/BAM file and they are
				# aligned then there will be no unaligned lines of a single read. and 
				# for a single read we get the same MAPQ for all mappings of it so we 
				# can get away with just checking the first alignment in the buffer
				# for aligned state or mapq state
				#
				...
				if process:
					...
					# new read name so we can push the buffer out to the queue
					tmpout, hit_state = process_read(aln_buffer, annot, lktable, ovl_ratio=args.min_overlap_ratio, strand_mode=strand_mode, 
						weight_hits=(not args.unweighted))
					hit_factory.update_hits([tmpout])
		...
		if len(aln_buffer) > 0:
			...
			if process:
				...
				tmpout, hit_state = process_read(aln_buffer, annot, lktable, ovl_ratio=args.min_overlap_ratio, 
					strand_mode=strand_mode, weight_hits=(not args.unweighted))			
				hit_factory.update_hits([tmpout])
...
class HitFactory(object):
	...
	def get_header(self):
		...
		if self.annot is not None:
			lout += ["gene_id", "gene_name", "chrom", "strand", "length"]
...
def process_read(item, annot, lktable, ovl_ratio=0.95, strand_mode=None, weight_hits=True):
...
#	item = item[0]
	...
	if pe:
		...
		if len(lpaired) > 0:
			...
			for i in range(len(lpaired)):
				...
				# find valid hit list for first mate
				hits1 = process_bounds(lpaired[i][0].bounds, lktable, strand_mode, lpaired[i][0].aln_len, ovl_ratio, mate=1)
				# find valid hit list for second mate				
				hits2 = process_bounds(lpaired[i][1].bounds, lktable, strand_mode, lpaired[i][0].aln_len, ovl_ratio, mate=2)
				
				# now take intersection of hits1 and hits2 and update the hits set for this read
				hits.update(list(set(hits1).intersection(set(hits2))))
		...
		for aln in aln1:
			...
			hits_tmp = process_bounds(aln.bounds, lktable, strand_mode, aln.aln_len, ovl_ratio)
			if len(hits_tmp) > 0:
				hits.update(hits_tmp)
	...
	if len(hits) > 0:
		...
		# calculate weight of alignment assignment. this is 1-over total targets hit
		# times 1-over total genes hit squared
		if weight_hits:

			for tid in hits:
				gid_hit.update([annot[tid]['gid']])
			...
			wi = 1.0/len(hits) * 1.0/(len(gid_hit)**2)
...
def process_bounds(lbounds, lktable, strand_mode, read_length, min_ovl_ratio, mate=1):
