...
def main(args):
	...
	for rfile in args.read_files:
		...
		if m:
			sp.call("bash -c 'bowtie --offrate 1 -t -p {:d} -a -n 1 -l 25 -e 60 -S alt_locs <(gunzip -c {:s}) | samtools view -bS -F 0x04 -o temp.bam -'".format(args.num_threads,rfile),shell=True)
		else:
		...
		for szl in p1.stdout:
			...
			if len(szl) > 0:
				...
				# check aloc id at this row
				if arl[0] != cid:
					# hit a new cid
					
					if cid != "":
						# if there was a previous cid assign its counts
						temp = position_count - pre_starts - post_ends
						if temp < 0:
							aloc_counts[cid] = 0
						else:
							aloc_counts[cid] = temp
						#}
						# print cid, ppos, aloc_counts[cid]
					#}
				...
				if cpos >= ppos - overlap_window and cpos <= ppos + overlap_window:
					#
					# we are within the window. if we are at the splice position we want the total count of 
					# hits (match or mismatch) minus any starts or ends of reads. otherwise if we 
					# are before the position we want a count of starts. if we are after the position we
					# want a count of ends.
					#
					pdata = arl[4]
					# drop insertions
					...
					# print cid, cpos, ppos,pdata

					if cpos < ppos:
						# count alignment starts
						i = 0
						while i < len(pdata):
							if pdata[i] == "^":
								pre_starts += 1
					...
					#}
						...
						# past the position, count alignment ends
						i = 0
						while i < len(pdata):
							if pdata[i] == "$":
								post_ends += 1
					...
					#}
						...
						# we are at the splice position - need the count at this spot minus
						# any starts or ends
						position_count = len(pdata)
						i = 0
						while i < len(pdata):
							if pdata[i] == "$":
								# skip ends of reads
	...
	# clean up
	p1 = sp.Popen("rm temp.bam temp.exons temp.juncs temp_locus.gtf".split())
	p1.wait()
