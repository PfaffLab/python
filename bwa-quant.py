...
# 20170517
# 
# multi-process version of bwa-quant. BWA alignments are split out 
# into several files (hard coded to be 16) and processed in parallel.
# each process produces its own quantification of the reads parsed
# and then those are merged when all processes are complete. 
#
...
def main(args):
	...
	#
	# main loop variables for alignment buffer files, handles, etc
	#
	...
	#
	# create sam files for dumping bundled alignments out to. also 
	...
	for i in range(args.t):
		...
		p = mp.Process(target=read_worker, args=(tasks, results, args, annot, lock))
		p.daemon = True
		p.start()
		pool.append(p)

	# open first buffer
	sam_fname = "{}.{}.sam".format(ftmp, sam_index)
	sam_handle = open(sam_fname, "w")
	
	#
	# read alignments and write bundled reads out 
	#
	...
	for szl in fin:
		...
		if rname != rlast:
			# new read name means we can increment read count
			num_reads += 1
			# increment chunk count
			chunk_count += 1
			
			if len(aln_buffer) > 0:
				# print alignment buffer out to a buffer file
				num_aligned += 1
				# write buffer to file
				sam_handle.write(buffer_to_string(aln_buffer) + "\n")
				# increment file index so we write to the next one next time
			
			# clear read buffer
			aln_buffer = []
			
			# check chunk count. if we hit our limit then we can close 
			# the current buffer files and push them into the task queue
			if chunk_count >= chunk_size:
				# close up and send to queue
				sam_handle.close()
				tasks.put(sam_fname)
				# make new one
				sam_index += 1
	...
	# wait...
	message("Waiting for children to finish...")
	tasks.join()
	for p in pool:
		p.join()
	...
	if has_gtf:
		...
		dheader = { 'length':1, 'eff_length':2, 'hits':3, 'eff_hits':4, 'fpkm':5 }
...
def buffer_from_string(sz):
	...
	while i < len(tmp):
		...
		buf[idx][SAM_FLAG] = int(buf[idx][SAM_FLAG])
		idx += 1
		i = i + 1 + n
...
# receives "sam" file names in the tasks queue. it has to open the file and
# process the bundled reads in there
def read_worker(task_queue, results_queue, args, annot, lock):
	# make a dict for hits picked up in this worker
	...
	frag_len_n = 1
	frag_len_mean = 200
	frag_len_stdev = frag_len_mean/2
	frag_len_mean0 = 0
	frag_len_stdev0 = 0
	...
	while True:
		...
		for szl in fin:
			try:
				buffer = buffer_from_string(szl)
			except:
				message_mp("buffer fail", name, lock)
				lock.acquire()
				print szl
				lock.release()
				continue
			...
			# deal with task which should just be a buffered read
			try:
				rres = process_read(args, hits, annot, buffer)
			except:
				message_mp("process fail", name, lock)
				lock.acquire()
				print buffer
				lock.release()
				continue
	...
	# write hits out to a process-specific file
	message_mp("assigned {}, fragment length {:0.1f} +/- {:0.1f}".format(num_assigned, frag_len_mean, sqrt(frag_len_stdev*1.0/(frag_len_n-1))), name, lock)
#	message_mp("Finished queue. Saving hits to file.", name, lock)
	fout_name = "{}.hits".format(name)
	fout = open(fout_name, "w")
	# put name of this hits file out into the results queue so we know it exists back
	# in the main process
	results_queue.put(fout_name)
	fout.write("num_assigned:{},num_assigned_unique:{},num_assigned_multi:{},frag:{},stdev:{},n:{}\n".format(num_assigned, num_assigned_unique, num_assigned_multi, frag_len_mean, sqrt(frag_len_stdev*1.0/(frag_len_n-1)), frag_len_n))
...
# named 'stub'.'mate'.sai. this function is assigned as 
# a target to a Process and takes in the read names 
# from a queue 'read_queue'
