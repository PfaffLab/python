...
#
# Cluster transcripts into connected clusters. Use common exon donor/accepter
# edges to join transcripts in a graph and then extract all of the 
# connected clusters. 
#==============================================================================
...
def main(args):
	...
	for ref in sorted(gtf.keys()):
		for strand in sorted(gtf[ref].keys()):
			...
			for idx in fook:
				gidx += 1
				gene_id = "{}_{:08d}".format(args.stub, gidx)
				for tid in foo[idx]:
					for grow in gtf[ref][strand][tid]: 
						grow['attrs']['gene_id'] = gene_id
